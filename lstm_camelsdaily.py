...
#change to 1d
#>conda activate mesh
#>python lstm_main.py --config config_lstm_camelsdaily.yaml
#date: 11/04/2025, this was used to generate LSTM 1-d results
#====================================================================
import math
import shutil
import os,sys
...
from camels_dataloader_daily import AllDataloader
...
def postprocess(cfg, seed=None, badstations=None):
    if seed is None:
        seed = cfg.seed
    metrics = pkl.load(open(f'camelsdaily_lstm_{cfg.model.seq_len}_{cfg.model.pred_len}_{seed}.pkl', 'rb'))
    NSE = []
    KGE = []
    R   = []
    FHV = []
    ...
    return (NSE[:,0], KGE[:,0], FHV[:,0], FLV[:,0])
...
def main(reTrain=False):
    ...
    if reTrain:
        ...
        regen= True
        if regen:
            testDataloader = allDataLoader.getDataLoader(mode='test', reload=False)
            model, testDataloader = accelerator.prepare(model, testDataloader)
            if accelerator.is_main_process:            
                accelerator.print(f"Loading best model {os.path.join(config_model.output_dir, f'bestmodel{cfg.seed}.pth')}")
                ...
                pkl.dump(metrics, open(f'camelsdaily_lstm_{config_model.seq_len}_{config_model.pred_len}_{cfg.seed}.pkl', 'wb'))
        else:
            postprocess(cfg)    
